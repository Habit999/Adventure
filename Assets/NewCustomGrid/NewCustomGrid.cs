...
public class NewCustomGrid : MonoBehaviour
{
	[HideInInspector] public NewGridData _gridData;
	...
	public void SpawnGrid()
	{
		...
		for(int axisX = 0; axisX < _gridLengthX; axisX++)
		{
			for(int axisZ = 0; axisZ < _gridLengthZ; axisZ++)
			{
				...
				NewGridCell cellScript = cellInstance.GetComponent<NewGridCell>();
	...
	public void UpdateGridFromCellData()
	{
		for(int x = 0; x < _gridLengthX; x++)
		{
			...
			{
				GeneratedData._activeCells[x, z] = GeneratedData._spawnedCells[x, z].GetComponent<NewGridCell>()._cellActive;
				GeneratedData._cellOccupantPrefabs[x, z] = GeneratedData._spawnedCells[x, z].GetComponent<NewGridCell>()._occupantPrefab;
				GeneratedData._cellOccupantPositions[x, z] = GeneratedData._spawnedCells[x, z].GetComponent<NewGridCell>()._occupantPosition;
				GeneratedData._cellOccupantEulerAngles[x, z] = GeneratedData._spawnedCells[x, z].GetComponent<NewGridCell>()._occupantEulerAngles;
				
				GeneratedData._spawnedCells[x, z].GetComponent<NewGridCell>().UpdateOccupantTransform();
			}
		}
	}
	...
	{
		NewGridData loadedData = LoadData();
	...
	void SaveData(NewGridData gData)
	{
	...
	NewGridData LoadData()
	{
		if(File.Exists(GridDataPath))
		{
			string jsonData = File.ReadAllText(GridDataPath);
			return _gridData = JsonUtility.FromJson<NewGridData>(jsonData);
		}
		else return null;
	}
